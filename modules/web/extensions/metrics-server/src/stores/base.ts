...
export default function BaseStore<T extends PathParams>({
  ...
  const fetchList = async (
    ...
    const result: any =
      // @ts-ignore
      (await request.get(getResourceUrl({ cluster, workspace, namespace, devops }, params), {
        params: params1,
        headers,
      })) ?? {};
  ...
  const fetchDetailWithoutWarning = async (params: PathParams) => {
    ...
    return !isEmpty(result) ? { ...params, ...formatFn(result) } : {};
  };
  ...
  const batchPatch = async (
    ...
    const promises = items.map(({ params, data, config, k8sVersion }) =>
      patch(params, data, config, k8sVersion),
    );
